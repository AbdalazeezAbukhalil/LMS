...
from APP.schemas.author_schemas import AuthorCreateSchema, AuthorUpdateSchema, AuthorReadSchema
...
async def get_author_details(author_id: UUID, service: AuthorService = Depends(get_author_service)):
...
async def create_author(author: AuthorCreateSchema, service: AuthorService = Depends(get_author_service)):
    entity = Author(
        id=uuid4(),
        name=author.name,
        bio=author.bio,
...
async def update_author(author_id: UUID, author: AuthorUpdateSchema, service: AuthorService = Depends(get_author_service)):
    author_details = await service.get_author_details(author_id)
    if not author_details:
        raise HTTPException(status_code=404, detail="Author not found")
    entity = Author(
...
async def delete_author(author_id: UUID, service: AuthorService = Depends(get_author_service)):
    author_details = await service.get_author_details(author_id)
    if not author_details:
        raise HTTPException(status_code=404, detail="Author not found")
