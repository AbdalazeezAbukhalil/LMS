...
class AuthorSQLRepository(AuthorRepository):
    ...
    async def get_authors(self) -> List[Author]:
        ...
        return [Author(id=a.id, name=a.name, bio=a.bio, created_at=a.created_at, updated_at=a.updated_at) for a in authors]

    async def get_author_details(self, author_id: UUID) -> Author:
        a = await self.session.get(AuthorModel, author_id)
        if a is None:
            return None
        return Author(id=a.id, name=a.name, bio=a.bio, created_at=a.created_at, updated_at=a.updated_at)
    ...
    async def create_author(self, author: Author) -> Author:
        ...
        return Author(id=db_author.id, name=db_author.name, bio=db_author.bio, created_at=db_author.created_at, updated_at=db_author.updated_at)
